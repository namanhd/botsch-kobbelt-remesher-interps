...
void equalize_valences(Eigen::PlainObjectBase<DerivedV_etc> &V_etc,
  ...
  for (int i = 0; i < k; i++) {
    ...
    int c = F(f1, c1);
    int d = F(f2, c2);
    if (is_feature_vertex[a] || is_feature_vertex[b] || is_feature_vertex[c] ||
        is_feature_vertex[d]) {
    ...
    if (!is_feature_edge) {
      // FIND VALENCES
      int deviation_pre =
          abs(vertex_valences(a) - 6) + abs(vertex_valences(b) - 6) +
          abs(vertex_valences(c) - 6) + abs(vertex_valences(d) - 6);
      // std::cout << i << std::endl;
      //  igl::flip_edge(V,F,E,uE,EMAP,uE2E,i);
      int deviation_post =
          abs(vertex_valences(a) - 1 - 6) + abs(vertex_valences(b) - 6 - 1) +
          abs(vertex_valences(c) - 6 + 1) + abs(vertex_valences(d) - 6 + 1);
      // std::cout << i << std::endl;
