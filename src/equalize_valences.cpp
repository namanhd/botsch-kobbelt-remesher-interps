...
void equalize_valences(Eigen::MatrixXd &V_etc, Eigen::MatrixXi &F, double selthresh, Eigen::VectorXi &feature)
{
    Eigen::VectorXd p;
    ...
    int m = F.rows();
    int n = V_etc.rows();
    const int n_attribs_minus1 = V_etc.cols() - 1; // to write selection value
    int a, b, c, d;
    igl::adjacency_list(E, A);
    Eigen::VectorXi vertex_valences;
    //
    ...
    std::function<void(
        ...
        int &)>
        flip_edge_adjacency = [&vertex_valences, selthresh, n_attribs_minus1, &V_etc, &A](
                                  Eigen::MatrixXi &F,                  // F
                                  Eigen::MatrixXi &E,                  // E
                                  ...
                                  int &uei) -> void
    {
        ...
        if (!(V_etc(uE(uei, 0), Eigen::last) >= selthresh && V_etc(uE(uei, 1), Eigen::last) >= selthresh)) {
            return;
        }
        ...
        if (!bad)
        {
            ...
            assert(uE(uei, 0) == v3);
            assert(uE(uei, 1) == v4);
            // so the resulting edge of the flip is (v3, v4) (they each gained 1 degree)
            // and we also set them to selected now
            double mean_sel_value = (V_etc(v1, Eigen::last) + V_etc(v2, Eigen::last)) / 2;
            ...
            std::remove_if(A[v1].begin(), A[v1].begin(), [&v2](const int &v)
                           { return v == v2; });
            std::remove_if(A[v2].begin(), A[v2].begin(), [&v1](const int &v)
                           { return v == v1; });
            A[v3].push_back(v4);
            A[v4].push_back(v3);
        }
