...
void split_edges_until_bound(
  ...
  while (keep_splitting) {
    ...
    for (int i = 0; i < uE.rows(); i++) {
      ...
      if (!(V_etc(uE(i, 0), Eigen::last) >= selthresh &&
            V_etc(uE(i, 1), Eigen::last) >= selthresh)) {
        continue;
      }
      // std::cout << "B" << std::endl;
      if (!is_feature_vertex[uE(i, 0)] && !is_feature_vertex[uE(i, 1)] &&
          uE2E[i].size() == 2) {
        // if (is_feature_vertex_vec(uE(i,0))==0 &&
