...
void split_edges_until_bound(Eigen::MatrixXd &V_etc, Eigen::MatrixXi &F, double selthresh, Eigen::VectorXi &feature, Eigen::VectorXd &high, Eigen::VectorXd &low)
{
    int m = F.rows();
    int n = V_etc.rows();
    ...
    while (keep_splitting)
    {
        ...
        {
            // skip considering this edge if not selected
            if (!(V_etc(uE(i, 0), Eigen::last) >= selthresh && V_etc(uE(i, 1), Eigen::last) >= selthresh)) {
                continue;
            }
            // std::cout << "B" << std::endl;
            if (!is_feature_vertex[uE(i, 0)] && !is_feature_vertex[uE(i, 1)] && uE2E[i].size() == 2)
            {
                ...
                if ((V_etc.row(uE(i, 0)).head<3>() - V_etc.row(uE(i, 1)).head<3>()).norm() > (std::min(high(uE(i, 0)), high(uE(i, 1)))))
                {
