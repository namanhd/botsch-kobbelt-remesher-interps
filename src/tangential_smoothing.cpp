...
void tangential_smoothing(Eigen::MatrixXd &V_etc, Eigen::MatrixXi &F,
  ...
  for (int i = 0; i < n; i++) {
    ...
    bool allow_this_vertex =
        V_etc(i, Eigen::last) >= selthresh && (!is_feature_vertex[i]);
    if (allow_this_vertex) {
      ...
      if (use_sizing_field_weighted_avg) {
        ...
        for (int j = 0; j < V2F[i].size(); j++) {
          double cur_denom =
              (area(V2F[i][j]) *
               (sizingField(F(V2F[i][j], 0)) + sizingField(F(V2F[i][j], 1)) +
                sizingField(F(V2F[i][j], 2))) /
               3);
        ...
        // so this is
        // sum_{f in adjacentFaces}((area of f * mean of sizing of the verts of
        // face f) * barycenter of face f) divided by (sum_{f in
        // adjacentFaces}(area of f * mean of sizing of the verts of face f))
        //  i.e. q is a weighted average of the barycenters of adjacent faces
        // (where the weight of a face is its area times its mean sizing field
        // value)
      ...
      // namanh NOTE: N is (nverts, 3), N.row(i) is RowVector3d, transpose gets
      // col Vector3d, then mult with row Vector 3d
      //  (N.row(i).transpose() * (N.row(i)))  is a (1,3).T @ (1,3) which is
      //  (3,1)@(1,3) which is (3,3)
      // so this is the outer product of the normal with itself...

      NN = lambda(i) * (Eigen::MatrixXd::Identity(3, 3) -
                        N.row(i).transpose() * (N.row(i)));
      ...
      // and then
      // p = v_etc[i] - (v_etc[i]-q - ((vnormals[i] dot (v_etc[i]-q)) .*
      // (v_etc[i]-q))) p = q + ((vnormals[i] dot (v_etc[i]-q)) .* (v_etc[i]-q))
      // lol what..
      ...
      //                    // std::cout << "Avoided face flipping, I think." <<
      //                    std::endl; V.row(i) = V_fixed.row(i);
      //                }
      //            }
    }
  }
