...
void calc_adaptive_sizing_field(
	Eigen::MatrixXd &V_etc, 
	Eigen::MatrixXi &F,
	Eigen::VectorXd &sizingField,
	double epsilon, 
	bool adaptive,
	double my_min_adaptive_sizing,
	double my_max_adaptive_sizing
)
{
	if (adaptive) {
		Eigen::VectorXd epsilon_vec = Eigen::VectorXd::Constant(V_etc.rows(), epsilon);
		...
		for (int i = 0; i < sizingField.size(); i++)  // clip sizingfield if it is below or above values
		{
			...
			// 	// double my_max = 4;
			// 	// double my_min = 0.01;
			if ((sizingField(i) > my_max_adaptive_sizing)) // || std::isnan(sizingField(i))
			{
	...
	Eigen::VectorXi &Vselection_out)
{
	...
	const double highlow_averager = 1/(1.4+0.7);
	...
	// stack Vselection_in onto Vattrs_in, forming V_etc that will be interpolated throughout
	...
	for (int i = 0; i < iters; i++)
	{
		...
		high = 1.4 * Vtargetlen_in;
		low = 0.7 * Vtargetlen_in;

		if (verbose) {
			std::cout << "splitting...\n";
		}
		split_edges_until_bound(V_etc, F_out, selection_threshold, feature, high, low); // Split

		if (verbose) {
			std::cout << "collapsing...\n";
		}
		collapse_edges(V_etc, F_out, selection_threshold, feature, high, low); // Collapse
		...
		tangential_smoothing(V_etc, F_out, selection_threshold, feature, lambda, (high + low) * highlow_averager, true); // Smooth
		...
		if (project)
		{
    		...
    		igl::point_mesh_squared_distance(V_etc.leftCols(3).eval(), V0, F0, sqrD, sqrI, V_projected);  // Project
	...
	Vselection_out = (V_etc.rightCols<1>().array() >= selection_threshold).cast<int>().matrix();
}
	...
	Eigen::VectorXi &Vselection_out)
{
	...
	for (int i = 0; i < iters; i++)
	{
		...
		int new_n_verts = V_etc.rows();
		// update this for the latest V_etc size which won't change anymore for the rest of this loop iteration
		...
		tangential_smoothing(V_etc, F_out, selection_threshold, feature, lambda, sizingField, true); // Smooth
	...
	Eigen::VectorXi &Vselection_out)
{
	...
	for (int i = 0; i < iters; i++)
	{
		...
		calc_adaptive_sizing_field(V_etc, F_out, sizingField, epsilon, adaptive, 0.01, 4.0);
		high = 1.4 * sizingField;
		low = 0.7 * sizingField;
