...
remesh_botsch_with_interps(
    ...
    const nb::DRef<const Eigen::MatrixXi> &F,
    const nb::DRef<const Eigen::VectorXi> &Vselection,
    const nb::DRef<const Eigen::VectorXd> &Vtargetlen,
    double selection_threshold,
    int iterations,
    bool project,
    int verbose
    ...
    Eigen::VectorXi Vselection_remesh;
    remesh_botsch(
        Vattrs, F, Vselection, Vtargetlen, 
        selection_threshold,
        iterations, 
        project, 
        verbose, 
...
remesh_botsch_adaptive_with_interps(
    ...
    Eigen::MatrixXi F_remesh;
    Eigen::VectorXi Vselection_remesh;
    remesh_botsch_adaptive(
        Vattrs, F, Vselection, epsilon, adaptive,
        selection_threshold,
        iterations, 
        project, 
        verbose, 
...
NB_MODULE(adremeshpy, m) {
    ...
    m.def("remesh_botsch_with_interps", 
        (std::tuple<Eigen::MatrixXd, Eigen::MatrixXi, Eigen::VectorXi>(*)(
            ...
            const nb::DRef<const Eigen::MatrixXi> &,
            const nb::DRef<const Eigen::VectorXi> &,
            const nb::DRef<const Eigen::VectorXd> &,
            double,
            int,
            bool,
            int 
    ...
    m.def("remesh_botsch_adaptive_with_interps", &remesh_botsch_adaptive_with_interps,
        nb::arg("Vattrs"), 
        nb::arg("F"),
        nb::arg("Vselection"),
        nb::arg("epsilon"),
        ...
        nb::arg("project"),
        nb::arg("verbose") = 0,
        "Adaptive isotropic remeshing, with target edge length/sizing field computed via curvature."
    );
}
