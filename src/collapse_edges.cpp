...
void collapse_edges(Eigen::MatrixXd &V_etc, Eigen::MatrixXi &F, double selthresh, Eigen::VectorXi &feature, Eigen::VectorXd &high, Eigen::VectorXd &low)
{
    ...
    // seems like in Eigen the assignment operator = does a copy so it's better to just
    // slice every time we need a [:, :3] i.e.  (Eigen::all, Eigen::seq(0,3)) or leftCols<3>()
    // which is actually zero-overhead
    ...
    int e_v1 = -1, e_v2 = -1;
    // criterion for collapse can be || and not && selected, but we should probably
    // only set the result to be selected if both were selected
    ...
    igl::decimate_post_collapse_callback post_collapse_lambda =
        [&V_etc, selthresh, n_attribs_minus1, &e_v1, &e_v2, &both_selected](
            const Eigen::MatrixXd &V , 
            const Eigen::MatrixXi &F , 
            const Eigen::MatrixXi &E , 
            const Eigen::VectorXi &EMAP , 
    ...
    igl::decimate(V_etc.leftCols<3>().eval(), F, 
        shortest_edge_and_midpoint_lambda, 
        stopping_condition, 
        pre_collapse_lambda,
        post_collapse_lambda, 
        U, G, J, I
    );
    // std::cout << "!!" << std::endl;
